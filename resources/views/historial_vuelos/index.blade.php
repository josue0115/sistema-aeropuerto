...
@section('page-title', 'Historial de Vuelos')
            ...
            <div class="material-card">
                ...
                <div class="card-header d-flex justify-content-between align-items-center" style="background: linear-gradient(135deg, var(--color-primary), var(--color-primary-light)); color: white; border: none; padding: 24px;">
                    ...
                    <div class="card-tools">
                        {{-- Botón Crear Nuevo Historial con estilo degradado --}}
                        <a href="{{ route('historial_vuelos.create') }}" class="material-btn material-btn-primary">
                    ...
                    <table id="tablaHistorial" class="table table-striped w-100">
                        ...
                        <tbody>
                            @forelse($historiales ?? [] as $historial)
                                <tr>
                                    <td style="font-weight: 600; color: var(--color-primary);">{{ $historial->id_historial_vuelo }}</td>
                                    <td>
                                        {{-- Combinamos ID Historial e ID Vuelo --}}
                                        <strong style="color: var(--color-primary-light);">Historial: {{ $historial->idhistorial }}</strong>
                                        <br>
                                        <span style="font-size: 0.9em; color: var(--color-text-muted);">Vuelo: {{ $historial->idvuelo }}</span>
                                    </td>
                                    <td>{{ $historial->idPasajero }}</td>
                                    <td>
                                        {{-- Formato de fecha local --}}
                                        <span style="font-weight: 500;">
                                            {{ $historial->Fecha ? \Carbon\Carbon::parse($historial->Fecha)->format('d/m/Y H:i') : 'N/A' }}
                                        </span>
                                    </td>
                                    <td style="max-width: 300px; white-space: normal;">
                                        {{-- Detalle del evento --}}
                                        {{ Str::limit($historial->Detalle, 70, '...') }}
                                    </td>
                                    <td style="white-space: nowrap;">
                                        <div class="btn-group" role="group">
                                            {{-- Botón Ver (Azul Claro - Info) --}}
                                            <a href="{{ route('historial_vuelos.show', $historial->id_historial_vuelo) }}" class="material-btn" style="background: linear-gradient(135deg, var(--color-info), #63b3ed); color: white; padding: 6px 12px; font-size: 0.8rem; margin-right: 4px;">
                                                <i class="material-icons" style="font-size: 1rem;">visibility</i>
                                            </a>
                                            
                                            {{-- Botón Editar (Naranja - Warning) --}}
                                            <a href="{{ route('historial_vuelos.edit', $historial->id_historial_vuelo) }}" class="material-btn" style="background: linear-gradient(135deg, var(--color-warning), #ed8936); color: white; padding: 6px 12px; font-size: 0.8rem; margin-right: 4px;">
                                                <i class="material-icons" style="font-size: 1rem;">edit</i>
                                            </a>
                                            {{-- Botón Eliminar (Rojo - Danger) --}}
                                            <form action="{{ route('historial_vuelos.destroy', $historial->id_historial_vuelo) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('DELETE')
