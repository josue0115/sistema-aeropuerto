...
@section('content')
{{-- Se elimina el 'max-height' y 'overflow-y: scroll' del contenedor principal para que lo maneje el layout si es necesario. --}}
<div class="container mx-auto px-4 py-8">
    
    <div class="mb-6"> {{-- Se aumentó el margen inferior para mejor separación --}}
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex space-x-3">
                ...
                <a href="{{ route('boletos.create') }}" class="material-btn material-btn-secondary flex items-center">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6"> {{-- Aumento de espacio --}}
        @if(isset($boletoCreado))
                ...
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600 text-sm">
                            **#{{ $boletoCreado->idBoleto }}** | Pasajero: **{{ $boletoCreado->idPasajero }}** | Vuelo: **{{ $boletoCreado->idVuelo }}**
                        </p>
                    </div>
                </div>
            </div>
            ...
            <form action="{{ route('servicios.store') }}" method="POST" id="servicio-form">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <select class="form-select w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('idBoleto') border-red-500 @enderror" id="idBoleto" name="idBoleto" required>
                            ...
                            @foreach($boletos as $boleto)
                                {{-- Si el boletoCreado ya está seleccionado, evitamos duplicar la opción, solo si no es el mismo que $boleto --}}
                                @if(!isset($boletoCreado) || $boleto->idBoleto != $boletoCreado->idBoleto)
                                    <option value="{{ $boleto->idBoleto }}" {{ old('idBoleto') == $boleto->idBoleto ? 'selected' : '' }}>
                                        {{ $boleto->idBoleto }} - Pasajero: {{ $boleto->idPasajero }}
                                    </option>
                                @endif
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" class="form-input w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('Fecha') border-red-500 @enderror" id="Fecha" name="Fecha" value="{{ old('Fecha', date('Y-m-d\TH:i')) }}" min="{{ date('Y-m-d\TH:i') }}" required>
                        @error('Fecha')
                            <div class="text-red-500 text-xs mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                {{-- Botón de Agregar Servicio estilizado con clases de Tailwind --}}
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 pt-6 border-t border-gray-200">
                    <div>
                        ...
                        <select class="form-select w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('Estado') border-red-500 @enderror" id="Estado" name="Estado" required>
                            ...
                            <option value="Activo" {{ old('Estado', 'Activo') == 'Activo' || !old('Estado') ? 'selected' : '' }}>Activo (Pagado/Confirmado)</option>
                            <option value="Inactivo" {{ old('Estado') == 'Inactivo' ? 'selected' : '' }}>Inactivo</option>
                            <option value="Pendiente" {{ old('Estado') == 'Pendiente' ? 'selected' : '' }}>Pendiente (Por Pagar)</option>
                        </select>
                        @error('Estado')
                            <div class="text-red-500 text-xs mt-1">{{ $message }}</div>
                        @enderror
                ...
                <div class="flex flex-row justify-end gap-4 mt-8 pt-6 border-t border-gray-200">
                    ...
                    {{-- Botón para Crear Servicios (si no viene de crear un boleto) --}}
                    @if(!isset($boletoCreado))
                    ...
                    {{-- Botón Siguiente: Asiento (si viene de crear un boleto) --}}
                    @if(isset($boletoCreado))
                    ...
                    <a href="{{ route('servicios.index') }}" class="material-btn material-btn-secondary flex-1 justify-center max-w-[300px]">
...
<template id="servicio-template">
    <div class="servicio-item flex items-end gap-4 mb-4 p-4 border border-gray-200 rounded-lg bg-white shadow-sm flex-wrap lg:flex-nowrap">
        ...
        <div class="flex-shrink-0 self-center"> {{-- Ajuste para centrar verticalmente --}}
