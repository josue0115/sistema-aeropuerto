                ...
                <section id="kpi-indicators" class="mb-12">
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div class="bg-white p-5 rounded-xl shadow-lg border border-primary/20 hover:shadow-xl transition duration-300">
                            ...
                            <p class="mt-1 text-3xl font-bold text-gray-900" id="totalVuelos">{{ number_format($vuelosDiarios) }}</p>
                        ...
                        <div class="bg-white p-5 rounded-xl shadow-lg border border-primary/20 hover:shadow-xl transition duration-300">
                            ...
                            <p class="mt-1 text-3xl font-bold text-gray-900" id="puntualidadSalidas">{{ $puntualidadSalidas }}%</p>
                        ...
                        <div class="bg-white p-5 rounded-xl shadow-lg border border-primary/20 hover:shadow-xl transition duration-300">
                            ...
                            <p class="mt-1 text-3xl font-bold text-gray-900" id="ingresosMensuales">${{ number_format($ingresosMensuales ) }} </p>
                            <p class="text-xs text-green-500 mt-1 flex items-center">
                                <i data-lucide="arrow-up" class="w-3 h-3 mr-1"></i>
                        ...
                        <div class="bg-white p-5 rounded-xl shadow-lg border border-primary/20 hover:shadow-xl transition duration-300">
                            ...
                            <p class="mt-1 text-3xl font-bold text-gray-900" id="npsScore">{{ $npsScore }}</p>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div class="mock-chart relative overflow-hidden">
                                ...
                                <svg class="absolute inset-0 w-full h-full p-2" viewBox="0 0 100 100" preserveAspectRatio="none">
                                    <polyline fill="none" stroke="#6366f1" stroke-width="2" points="
                                        0,{{ 100 - ($evolucionPuntualidad[6] ?? 0) }}
                                        16.66,{{ 100 - ($evolucionPuntualidad[5] ?? 0) }}
                                        33.32,{{ 100 - ($evolucionPuntualidad[4] ?? 0) }}
                                        49.98,{{ 100 - ($evolucionPuntualidad[3] ?? 0) }}
                                        66.64,{{ 100 - ($evolucionPuntualidad[2] ?? 0) }}
                                        83.3,{{ 100 - ($evolucionPuntualidad[1] ?? 0) }}
                                        99.96,{{ 100 - ($evolucionPuntualidad[0] ?? 0) }}
                                    " />
                                    <circle cx="99.96" cy="{{ 100 - ($evolucionPuntualidad[0] ?? 0) }}" r="2" fill="#6366f1" />
                                </svg>
                        ...
                        <div class="space-y-4">
                            <div class="bg-white p-5 rounded-xl shadow-lg border border-gray-200">
                                ...
                                <div class="text-2xl font-bold text-gray-900 my-1">{{ $capacidadPromedio }}%</div>
                                <div class="bg-gray-200 rounded-full h-2.5">
                                    <div class="progress-bar bg-primary h-2.5 rounded-full" style="width: {{ $capacidadPromedio }}%;"></div>
                                </div>
                            </div>
                            ...
                            <div class="bg-white p-5 rounded-xl shadow-lg border border-gray-200">
                                ...
                                <p class="text-2xl font-bold text-gray-900">{{ $tiempoEmbarque }} min</p>
                            ...
                            <div class="bg-white p-5 rounded-xl shadow-lg border border-gray-200">
                                ...
                                <p class="text-2xl font-bold text-gray-900">{{ $puertasActivas }} / {{ $puertasTotal }}</p>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="lg:col-span-1 bg-white p-6 rounded-xl shadow-lg">
                            ...
                            <div class="mock-chart relative flex justify-around items-end p-2">
                                @foreach($topDestinos as $index => $destino)
                                    @php
                                        ...
                                        $opacity = 1 - ($index * 0.2);
                                    @endphp
                                    <div class="mock-bar bg-primary" style="height: {{ $height }}%; left: {{ $left }}%; opacity: {{ $opacity }};">
                                        <span class="absolute -top-6 text-xs font-semibold text-primary">{{ number_format($destino->total) }}K</span>
                                    </div>
                                @endforeach
                            </div>
                            <div class="mt-4 text-xs text-center space-x-2">
                                @foreach($topDestinos as $destino)
                                    <span class="font-medium">{{ substr($destino->NombreAeropuerto, 0, 3) }}</span> @if(!$loop->last)·@endif
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="lg:col-span-2 bg-white p-6 rounded-xl shadow-lg">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                                <div class="border p-4 rounded-lg bg-gray-50">
                                    ...
                                    <p class="text-xl font-bold text-gray-900">${{ number_format($ingresosEconomica) }}M</p>
                                    <p class="text-sm text-primary">Avg: ${{ number_format($avgEconomica) }}</p>
                                </div>
                                <div class="border p-4 rounded-lg bg-gray-50">
                                    <p class="text-sm text-gray-500">Ejecutiva</p>
                                    <p class="text-xl font-bold text-gray-900">${{ number_format($ingresosEjecutiva) }}M</p>
                                    <p class="text-sm text-secondary">Avg: ${{ number_format($avgEjecutiva) }}</p>
                                </div>
                                <div class="border p-4 rounded-lg bg-gray-50">
                                    <p class="text-sm text-gray-500">Primera</p>
                                    <p class="text-xl font-bold text-gray-900">${{ number_format($ingresosPrimera ) }}M</p>
                                    <p class="text-sm text-accent">Avg: ${{ number_format($avgPrimera) }}</p>
                                </div>
                            </div>
                            ...
                            <div class="flex space-x-2 text-sm text-white font-semibold rounded-lg overflow-hidden">
                                <div class="bg-primary/90 p-2 text-center" style="flex: {{ $porcentajeRecurrentes }};">{{ $porcentajeRecurrentes }}% Recurrentes</div>
                                <div class="bg-secondary/90 p-2 text-center" style="flex: {{ $porcentajeNuevos }};">{{ $porcentajeNuevos }}% Nuevos</div>
                            </div>
                        </div>
                ...
                <section id="realtime-monitoring" class="mb-12">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="bg-white p-6 rounded-xl shadow-lg">
                            ...
                            <p class="text-sm text-gray-500 mt-3">Capacidad de Terminal A: <span class="text-primary font-bold">{{ $capacidadPromedio }}% ({{ intval($capacidadPromedio * 6.6) }}/660 personas)</span></p>
                        </div>
                        ...
                        <div class="bg-white p-6 rounded-xl shadow-lg overflow-x-auto">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($vuelosRecientes as $vuelo)
                                    <tr>
                                        <td class="px-3 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $vuelo['id'] }}</td>
                                        <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-500">{{ $vuelo['destino'] }}</td>
                                        <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-500">{{ $vuelo['puerta'] }}</td>
                                        <td class="px-3 py-4 whitespace-nowrap">
                                            @if($vuelo['estado'] == 'Embarcando')
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                                                    {{ $vuelo['estado'] }}
                                                </span>
                                            @elseif($vuelo['estado'] == 'Retrasado (30m)')
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">
                                                    {{ $vuelo['estado'] }}
                                                </span>
                                            @elseif($vuelo['estado'] == 'En Vuelo')
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="bg-white p-6 rounded-xl shadow-lg overflow-x-auto">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($rankingAerolineas as $aerolinea)
                                    <tr>
                                        <td class="px-3 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $aerolinea['nombre'] }}</td>
                                        <td class="px-3 py-4 whitespace-nowrap text-sm text-green-600 font-medium">{{ $aerolinea['puntualidad'] }}</td>
                                        <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-900">{{ $aerolinea['ventas'] }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        ...
                        <div class="bg-white p-6 rounded-xl shadow-lg">
                            ...
                            <div class="mock-chart relative flex justify-around items-end p-2">
                                @foreach($desempenoRutas as $index => $ruta)
                                    @if($index < 2)
                                        <!-- Alto Desempeño -->
                                        <div class="mock-bar bg-green-500/80" style="height: {{ 85 - ($index * 15) }}%; left: {{ 10 + ($index * 15) }}%;">
                                            <span class="absolute -top-6 text-xs font-semibold text-green-500">{{ $ruta['codigo'] }}</span>
                                        </div>
                                    @else
                                        <!-- Bajo Desempeño -->
                                        <div class="mock-bar bg-red-500/80" style="height: {{ 40 - (($index - 2) * 15) }}%; left: {{ 55 + (($index - 2) * 15) }}%;">
                                            <span class="absolute -top-6 text-xs font-semibold text-red-500">{{ $ruta['codigo'] }}</span>
                                        </div>
                                    @endif
                                @endforeach
                            </div>
