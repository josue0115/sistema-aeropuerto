...
@section('page-title', 'Reporte de Disponibilidad de Boletos')
        ...
        <div class="col-12">
            <div class="material-card">
                ...
                <div class="card-body" style="padding: 24px;">
                    ...
                    <form method="POST" action="{{ route('reportes.disponibilidad-boletos.generar') }}" class="p-4 rounded-lg mb-5" style="background-color: var(--color-background-light); border: 1px solid #dee2e6;">
                        @csrf
                        <div class="row g-3 align-items-end">
                            <div class="col-md-4">
                                <label for="fecha" class="form-label text-sm font-weight-bold text-secondary mb-1">Fecha</label>
                                <input type="date" id="fecha" name="fecha" value="{{ old('fecha', $fecha ?? date('Y-m-d')) }}"
                            ...
                            <table class="report-table table table-striped table-hover w-100">
                                ...
                                <tbody>
                                    @forelse($reporte as $item)
                                        <tr>
                                            <td class="font-weight-bold" style="color: var(--color-primary);">{{ $item->IdVuelo }}</td>
                                            <td>
                                                <i class="material-icons" style="font-size: 1rem; vertical-align: middle; color: var(--color-primary-light);">flight_takeoff</i>
                                                **{{ $item->origen }}** <i class="material-icons" style="font-size: 1rem; vertical-align: middle; color: var(--color-text-muted);">arrow_forward</i>
                                                **{{ $item->destino }}**
                                            </td>
                                            <td>
                                                Salida: {{ \Carbon\Carbon::parse($item->FechaSalida)->format('d/m/Y H:i') }}
                                                <br>
                                                Llegada: {{ \Carbon\Carbon::parse($item->FechaLlegada)->format('d/m/Y H:i') }}
                                            </td>
                                            <td class="text-center">{{ $item->Capacidad }}</td>
                                            <td class="text-center">{{ $item->boletos_vendidos }}</td>
                                            {{-- Lógica de color para disponibilidad --}}
                                            <td class="text-center font-weight-bold" style="font-size: 1.1rem; color: {{ $item->boletos_disponibles > 50 ? 'var(--color-success)' : ($item->boletos_disponibles > 0 ? 'var(--color-warning)' : 'var(--color-danger)') }}">
                                                {{ $item->boletos_disponibles }}
                                                @if($item->boletos_disponibles <= 0)
                            ...
                            <div class="mt-4 text-sm bg-light p-4 rounded-lg" style="border: 1px dashed #ced4da;">
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <span class="badge bg-secondary">{{ $reporte->count() }}</span>
                                    </div>
...
function exportDisponibilidad() {
    ...
    const url = `{{ route('reportes.disponibilidad-boletos.exportar') }}?fecha=${fecha}&id_vuelo=${idVuelo}`;
